...
public class TodoHsyController {
	...
	public ModelAndView requiredLogin_employeeTodo(HttpServletRequest request, HttpServletResponse response, ModelAndView mav) {
		...
		// 2) 업무관리페이지에서 보여줄 정보 페이징바 처리해서 정보 가져오기
		...
		// 로그인유저의 사원번호와 requiredState를 paraMap에 추가하기
		...
		if(pageNo != 1) {
			pageBar += "&nbsp;<a href='employeeTodo.tw?currentShowPageNo=1&requiredState="+requiredState+">[맨처음]</a>&nbsp;"; 
			pageBar += "&nbsp;<a href='employeeTodo.tw?currentShowPageNo="+(pageNo-1)+"&requiredState="+requiredState+"'>[이전]</a>&nbsp;";
		}
		...
		while(!(loop>blockSize || pageNo > totalPage) ) { // totalPage가 0인경우 페이지바는 생성되지 않는다
			if(pageNo == Integer.parseInt(currentShowPageNo)) {
				...
				pageBar += "&nbsp;<a href='employeeTodo.tw?currentShowPageNo="+pageNo+"&requiredState="+requiredState+"'>"+pageNo+"</a>&nbsp;";
			}
			loop++;
			pageNo++;
		}// end of while-----------------------------
		...
		if(pageNo <= totalPage) {
			pageBar += "&nbsp;<a href='employeeTodo.tw?currentShowPageNo="+pageNo+"&requiredState="+requiredState+"'>[다음]</a>&nbsp";
			pageBar += "&nbsp;<a href='employeeTodo.tw?currentShowPageNo="+totalPage+"&requiredState="+requiredState+"'>[마지막]</a>&nbsp";
		}
		
		mav.addObject("pageBar", pageBar);
		
		
		// 4) 신규등록업무, 진행중업무, 진행완료업무 총 건수 표시
