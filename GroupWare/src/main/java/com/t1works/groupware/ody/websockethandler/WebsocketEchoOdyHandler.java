...
public class WebsocketEchoOdyHandler extends TextWebSocketHandler {
 ...
 // === 클라이언트가 웹소켓서버에 연결했을때의 작업 처리하기 ===
    ...
    */
    ...
    //   ~~~확인용 connectingUserName: 「오다윤 김유신 」
   ...
   // === 클라이언트가 웹소켓 서버로 메시지를 보냈을때의 Send 이벤트를 처리하기 ===
    ...
    protected void handleTextMessage(WebSocketSession wsession, TextMessage message) 
      ...
      */ 
       ...
       System.out.println("====> 웹채팅확인용 : 로그인ID : " + loginuser.getEmployeeid());
       // ====> 웹채팅확인용 : 로그인ID : dayoon1013
                ...
                if (!wsession.getId().equals(webSocketSession.getId())) {  
                   ...
                   // wsession.getId() 와  webSocketSession.getId() 는 자동증가되는 고유한 숫자로 나옴 
                    webSocketSession.sendMessage(
                            new TextMessage("<span class='ip' style='display: none;'>"+wsession.getRemoteAddress().getAddress().getHostAddress()+"</span><span class='loginuserid' style='display: none;'>"+loginuser.getEmployeeid()+"</span>&nbsp;<p style='font-size:11pt; ' class='loginuserName'>" +loginuser.getName()+ "</p>" + "<span style='background-color: #fff; padding: 3px 2px;'>"+messageVO.getMessage()+"</span>"));  
                }
            } 
                   ...
                   // messageVO.getTo() 는 클라이언트가 보내온 귓속말대상 IP address 임.
                    webSocketSession.sendMessage(
                            new TextMessage(
                                    ...
                                    "<span class='ip' style='display: none;'>"+ wsession.getRemoteAddress().getAddress().getHostAddress() +"</span><span class='loginuserid' style='display: none;'>"+loginuser.getEmployeeid()+"</span>&nbsp;<p style='font-size:11pt;' class='loginuserName'>" +loginuser.getName()+ "</p>"  + "<span style='background-color: #fff; padding: 3px 2px;'>"+messageVO.getMessage()+"</span>"
                                   ) 
                    );
