...
=======
   <div id="sidemenu">
      ...
      <p id="salary" onclick="location.href='<%= ctxPath%>/t1/salary.tw'">월급관리</p>
      <c:choose>
          <c:when  test="${loginuser.fk_pcode eq '1'}">
              <p id="toDo" onclick="location.href='<%= ctxPath%>/t1/employeeTodo.tw'">업무관리</p>
          </c:when>
          <c:when  test="${loginuser.fk_dcode eq '4' && loginuser.fk_pcode eq '3'}">
              <p id="toDo" onclick="location.href='<%= ctxPath%>/t1/personnelManage.tw'">업무관리</p>
          </c:when>
          <c:when  test="${loginuser.fk_pcode eq '3' && (loginuser.fk_dcode eq '1' || loginuser.fk_dcode eq '2' || loginuser.fk_dcode eq '3')}">
              <p id="toDo" onclick="location.href='<%= ctxPath%>/t1/leaderTodo.tw'">업무관리</p>
          </c:when>
          <c:when test="${loginuser.fk_pcode eq'4'}">
              <p id="toDo" onclick="location.href='<%= ctxPath%>/t1/companyPerformance.tw'">업무관리</p>
          </c:when>
       </c:choose>
      <p id="reservation" onclick="location.href='<%= ctxPath%>/t1/rsRoom.tw'">예약관리</p>
   </div>
